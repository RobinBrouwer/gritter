module Gritter
  module Helpers
    def add_gritter text, *args
      ...
      options[:image] = (Rails.application.assets ? asset_path("#{options[:image]}#{options[:image].to_s == 'progress' ? '.gif' : '.png'}")  : "/images/gritter/#{options[:image]}#{options[:image].to_s == 'progress' ? '.gif' : '.png'}") if %w(success warning error notice progress).include?(options[:image].to_s)
